const date_picker_element = document.querySelector(".date-picker-container");
...
function populateDates(e) {
	...
	for (let index = 0; index < totalDays; index++) {
		...
		single_day_element.addEventListener("click", () =>
			userSelectDate(index)
		);
		days_element.appendChild(single_day_element);
	}
}

function userSelectDate(index) {
	...
	selectedMonth = currentMonth;
	selectedYear = currentYear;
	let currSelectedDay = selectedDay < 10 ? "0" + selectedDay : selectedDay;

	current_date_element.textContent =
		months[selectedMonth] + " " + currSelectedDay + ", " + selectedYear;
